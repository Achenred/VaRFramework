...
class Evaluator():
    def __init__(self,env,dir="logs/"):
        ...
        self.labels = ["var","naive_bcr_l1","naive_bcr_linf","weighted_bcr_l1","weighted_bcr_linf","mean_model","asymptoticvar","cvar","softrobust","worstrobust","naive_hoeffding","optimized_hoeffding"]
        self.flabels = ["var","naive_bcr_l1","naive_bcr_linf","weighted_bcr_l1","weighted_bcr_linf","asymptoticvar","cvar","softrobust","worstrobust","naive_hoeffding","optimized_hoeffding"]

        self.map = {"weighted_bcr_l1":"WBCR $l_1$","naive_bcr_l1":"BCR $l_1$","weighted_bcr_linf":"WBCR $l_{\infty}$","var":"VaR","naive_bcr_linf":"BCR $l_{\infty}$","mean_model": "Mean","asymptoticvar": "VaRN","cvar":"CVaR","softrobust":"Soft-Robust","worstrobust":"Worst-case RMDP","naive_hoeffding":"Naive Hoeffding" ,"optimized_hoeffding": "Optimized Hoeffding"}
        self.deltas = [0.3,0.15,0.05]
        self.fdeltas = [0.3,0.15,0.05]
        # self.fdeltas = [0.25,0.05]
    ...
    def load_all(self,dir="logs/"):
        ...
        for sub_dir in os.listdir(dir):
            ...
            domain = sub_dir.split("-")[0].replace("new","")
            if domain=="populatio":
                print("here")
            ...
            for file in os.listdir(path):
                ...
                if "true_returns" in path_:
                    ...
                    if true_res[domain].get(delta) is None:
                        true_res[domain][delta]=[]
                    true_res[domain][delta].append(rets)
                else:
                    ...
                    rets = np.load(path_)
                    res[domain][type][delta][method].append(rets)
        ...
        for method in self.flabels:
            try:
                ...
                for delta in self.fdeltas:
                    ...
                    val = np.quantile(ret,delta,axis=1)
                    # val = np.round(val,2)
                    vals.append(np.mean(val))
                    if not ("inventory" in fname and "hoeffding" in method):
                        results[method].append((delta,np.round(np.mean(val),2),np.round(np.std(val),2)))
                    else:
                        results[method].append((delta,np.mean(val),np.std(val)))


                    if (delta== 0.15):
                        line += str(np.mean(val)) +" + " + str(np.std(val)) +" & "
                    if (delta== 0.05):
                        line2 += str(np.mean(val)) +" + " + str(np.std(val)) + " & "
            except:
                print("fname",fname)
                pass
            vals = np.array(vals)
...
def print_all_results(all_results,type="test"):

    domains=["riverswim", "inventory", "population_small", "population"]
    percentiles=[0.3,0.15,0.05]
    idx=0
    flabels = ["var", "naive_bcr_l1", "naive_bcr_linf", "weighted_bcr_l1", "weighted_bcr_linf", "asymptoticvar",
                    "cvar", "softrobust", "worstrobust", "naive_hoeffding", "optimized_hoeffding"]
    map = {"weighted_bcr_l1": "WBCR $l_1$ [1]", "naive_bcr_l1": "BCR $l_1$ [2]", "weighted_bcr_linf": "WBCR $l_{\infty}$ [1]",
                "var": "VaR", "naive_bcr_linf": "BCR $l_{\infty}$ [2]", "mean_model": "Mean", "asymptoticvar": "VaRN",
                "cvar": "CVaR", "softrobust": "Soft-Robust [4]", "worstrobust": "Worst RMDP [3]",
                "naive_hoeffding": "Naive Hoeffding [5]", "optimized_hoeffding": "Optimized Hoeffding [2]"}
    ...
    for percentile in percentiles:
        ...
        for label in flabels:
            ...
            for domain in domains:
                ...
                try:
                    mean = all_results[domain][type].get(label)[idx][1]
                    var = all_results[domain][type].get(label)[idx][2]
                    r = str(mean) + " \pm " + str(var)
                except:
                    r="-"
...
if __name__=='__main__':
    ...
    for domain, val in res.items():
        ...
        if all_results.get(domain) is None:
            all_results[domain]={}
        for type, rets in val.items():
            name = domain  + "-"+ str(type)
            # eval.plot_returns(rets,name)

            percentile_values = eval.plot_percentile_returns(rets, true_val,name)
            if all_results.get(type) is None:
                all_results[domain][type]=percentile_values
        # eval.plot_bar_percentile_returns(res[domain]['train'],res[domain]['test'],true_val,domain)

    print_all_results(all_results,"test")
